...
const {rl, getRemainingAuctionTime, searchAuctionItem, registerAuction, immediateOrderOrBid } = require('./utils');
                ...
                rl.question('\n 1. 경매에 물품 등록하기 2. 메뉴 선택 돌아가기', async (answer) => {
                    switch (answer) {
                        case "1":
                            ...
                            registerAuction(recursiveAsyncReadLine, socket, token, errorHandler)
                            break;
                        case "2":
                            console.log("메뉴 선택으로 돌아갑니다");
                    ...
                    socket.on('ojt:auctionList', (res) => {
                        ...
                        if (res.action === 'registerAuction') {
                            ...
                            if (res.data.auctionList.length > 0) {
                                ...
                                immediateOrderOrBid(recursiveAsyncReadLine, auctionList, socket, token, errorHandler) // 검색 결과 나오면 1.즉시 구매 2.입찰 3.메뉴로 돌아가기 선택지 나옴
                            } else {
